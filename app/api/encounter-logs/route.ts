...
export async function POST(request: Request) {
  try {
    const { type, title, description, fields, author, timestamp, notes, evidence_url, friends_killed, enemies_killed, war_id } = await request.json();

    console.log('Received encounter log request:', { type, title, description, fields, author });
      ...
      if (war_id) {
        ...
        if (typeof war_id === 'string' && !isUuid(war_id)) {
          const { data, error } = await supabaseAdmin
            .from('faction_wars')
            .select('id, enemy_faction, started_at, slug')
            .eq('slug', war_id)
            .single();
